...
function conky_top_cpu_colour(arg)
	...
	local str = conky_parse(string.format('${top name %i}, tonumber(arg)))
	...
	print conky_top_colour(cpu, 0x008000, 15, 25)
--	return conky_top_colour(cpu, 0x008000, 15, 25)
end

-- parses the output from top and calls the colour function
function working_conky_top_cpu_colour(arg)
	-- input is the top var number we want to use

	local str = conky_parse(string.format('${top name %i} ${top pid %i}${alignr}${top cpu %i}${alignr}${top mem %i}${alignr}', tonumber(arg), tonumber(arg), tonumber(arg), tonumber(arg)))
